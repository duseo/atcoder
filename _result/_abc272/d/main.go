...
func main() {
    ...
    dist:=tdm(n,n,INF)
    push := func(i,j,d int) {
        if i < 0 || j < 0 || i >= n || j >=n || dist[i][j] != INF {
            return
        }
        dist[i][j] = d
        q = append(q, pair{i,j})
...
func printTdm(tdm [][]int, n int) {
    for i := 0; i < n; i++ {
        fmt.Println(intSliceToString(tdm[i]))
    }
}
