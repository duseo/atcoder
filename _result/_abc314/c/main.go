...
func main() {
	...
	s := []byte(ns())
	ci := make(map[int][]int)
	indexToColor := make(map[int]int)
	indexToMapPos := make(map[int]int)
	for i := 0; i < n; i++ {
		c := ni()
		indexToColor[i] = c
		ci[c] = append(ci[c], i)
		indexToMapPos[i] = len(ci[c]) - 1
	}

	getBefore := func(mp int, ci []int) int {
		if mp == 0 {
			return len(ci) - 1
		}
		return mp - 1
	...
	for i := 0; i < n; i++ {
		...
		newIndex := indices[getBefore(mapPos, ci[color])]
		res[i] = s[newIndex]
	}
