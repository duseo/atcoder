...
func main() {
	...
	for i := 0; i < n; i++ {
		f, d := ni2()
		diff[f] = max(diff[f], d)
		same[f] = append(same[f], d)
	}

	getEvenDiff := func() int {
		ma := 0
		for _, ints := range same {
			if len(ints) < 2 {
				continue
			}
			sort.Ints(ints)
			ma = max(ma, max(ints[len(ints)-1], ints[len(ints)-2])+(min(ints[len(ints)-1], ints[len(ints)-2])/2))
		}
		return ma
	}
	...
	fmt.Println(max(getMaxDiff(), getEvenDiff()))
