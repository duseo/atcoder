...
func main() {
	...
	for i := 0; i < n; i++ {
		...
		products = append(products, product{p, tmp})
	}
	for i := 0; i < n; i++ {
		for j := 0; j < n; j++ {
			if i != j {
				...
				hasAll := func(p1, p2 product) bool {
					for i2, _ := range p1.functions {
						if !p2.functions[i2] {
							return false
						}
					}
				...
				hasMore := func(p1, p2 product) bool {
					for i2, _ := range p2.functions {
						if !p1.functions[i2] {
							return true
						}
					}
				...
				if p1.price >= p2.price && hasAll(p1, p2) && (p1.price > p2.price || hasMore(p1, p2)) {
					printyn(true)
					return
				}
			}
