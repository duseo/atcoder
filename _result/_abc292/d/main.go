...
func main() {
	...
	for k, _ := range ids {
		vertices[k] = make(map[int]bool)
	}
	nedges := make(map[int]int)
	for i := 0; i < m; i++ {
		comp := uf.find(edges[i].x)
		vertices[comp][edges[i].x] = true
		vertices[comp][edges[i].y] = true
		nedges[comp]++
	}

	for k, _ := range ids {
		if len(vertices[k]) != nedges[k] {
			printyn(false)
			return
		}
	}
